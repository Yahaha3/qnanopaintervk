...
void QNanoPainter::opengl_init_create()
{
...
#ifdef BUILD_VULKAN_BACKENDS
void QNanoPainter::vulkan_init_create(QVulkanInstance *instance, QVulkanWindow *window)
{
    m_backend.reset(QNanoBackendFactory::createBackendVk(instance, window));
    qDebug() << "Using backend:" << m_backend->backendName();
    m_nvgContext = m_backend->nvgCreate(NVG_ANTIALIAS);
    Q_ASSERT_X(m_nvgContext, "QNanoPainter::QNanoPainter", "Could not init nanovg!");
...
void QNanoPainter::draw_polygon(const QVector<QPoint> &vertices)
{
    moveTo(vertices.first());
    for(auto i = 1, si = vertices.size(); i < si; i++)
    {
        lineTo(vertices.at(i));
    }
}
...
void QNanoPainter::updateImage(QNanoImage &image)
{
    auto id = image.getID(nvgCtx());
    auto array = image.m_array;
    auto length = array.length();
    unsigned char* data = reinterpret_cast<unsigned char*>(&array.data()[0]);
    nvgUpdateImageMem(nvgCtx(), id, data, length);
}
